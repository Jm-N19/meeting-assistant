...
class InsightAgent:
    ...
    async def process(self, state: dict) -> dict:
        ...
        try:
            ...
            result: dict = {"insights": insights, "errors": errors}
            return result
    ...
    def _compute_speaker_stats(
        ...
        result.sort(key=lambda x: x.duration_s, reverse=True)
        return result
    ...
    async def _analyze_with_llm(
        ...
        """调用 LLM 进行语义分析"""
        stats_text = "\n".join(
            f"- {s.speaker}: 发言{s.duration_s}秒, "
            f"占比{s.percentage:.1f}%, 发言{s.segment_count}次"
    ...
    def _compute_efficiency_score(
        ...
        # 发言均衡度评分（基于基尼系数的简化版）
        ratios = [s.percentage / 100 for s in speaker_stats]
        n = len(ratios)
        if n > 1:
        ...
        if transcript and transcript.duration_seconds > 0:
            total_speaking = sum(s.duration_s for s in speaker_stats)
